...
interface StandardBatchCompletionModalProps {
...
export default function StandardBatchCompletionModal({
  batch,
  isOpen,
  onClose,
  onComplete,
}: StandardBatchCompletionModalProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(
        `/api/admin/tasks/standard-clothing-batches/${batch.task_id}/complete`,
        {
          method: 'POST',
      ...
      <div className="flex min-h-full items-center justify-center p-4">
        <div className="relative bg-white rounded-lg shadow-xl max-w-md w-full">
            ...
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-lg font-semibold text-gray-900">
                  Complete Standard Batch
                </h2>
                <p className="text-sm text-gray-600 mt-1">
                  {batch.batch_id} - {batch.week_start} to {batch.week_end}
                </p>
              </div>
              <button
                onClick={onClose}
          ...
          <div className="px-6 py-4">
            {/* Order Summary */}
            <div className="mb-6">
              <h3 className="text-sm font-semibold text-gray-700 mb-2">Batch Summary</h3>
              <div className="bg-gray-50 rounded-lg p-3 space-y-1 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">{batch.event_names.length}</span>
                </div>
                <div className="flex justify-between pt-2 border-t">
                  <span className="text-gray-600">Customer Revenue:</span>
                  <span className="font-medium">{formatCurrency(batch.total_revenue)}</span>
                </div>
              </div>
            </div>
            ...
            <div className="mb-4">
              ...
              <textarea
                ...
                placeholder="Add any notes about this batch order..."
                disabled={isSubmitting}
                rows={3}
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 text-sm text-blue-800">
              ...
              <ul className="list-disc list-inside space-y-1 text-blue-700">
                <li>GO-ID for supplier order tracking (linked to all {batch.event_names.length} schools)</li>
                <li>Shipping task for delivery</li>
              </ul>
            </div>
