...
/**
 * Generate email body HTML for early bird reminder.
 * Returns only body content — the shared email wrapper adds header, footer, and impressum.
 */
function getEmailBodyHtml(recipient: CampaignRecipient): string {
  return `<p style="margin: 0 0 20px 0; color: #2F4858; font-size: 18px; font-weight: 600;">
  Halli Hallo,
</p>
...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const recipient of recipients) {
      ...
      const html = getEmailBodyHtml(recipient);
