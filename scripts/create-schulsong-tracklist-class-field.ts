/**
 * Script to create the schulsong_tracklist_class field on the Events Airtable table
 * and update the field ID in the codebase.
 *
 * Run with: npx tsx scripts/create-schulsong-tracklist-class-field.ts
 *
 * Creates 1 field:
 *   1. schulsong_tracklist_class (singleLineText) - custom class/school name for schulsong on CD booklet
 *
...
const FIELDS_TO_CREATE = [
  {
    name: 'schulsong_tracklist_class',
    type: 'singleLineText',
...
async function main() {
  ...
  console.log('Creating schulsong_tracklist_class field on Events table');
