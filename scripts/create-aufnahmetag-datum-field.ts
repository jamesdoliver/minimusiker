/**
 * Script to create aufnahmetag_datum date field in the Schulsong Airtable table
 * and update the field ID placeholder in the codebase.
 *
 * Run with: npx tsx scripts/create-aufnahmetag-datum-field.ts
 */
...
const FIELD_DEF = {
  name: 'aufnahmetag_datum',
  type: 'date' as const,
  description: 'Date of the recording day (Aufnahmetag)',
  options: {
    dateFormat: { name: 'iso' as const },
  },
  placeholder: 'fldTODO_aufnahmetag_datum',
};
...
async function main() {
  ...
  console.log('Creating aufnahmetag_datum field in Schulsong table');
  console.log(`Table: ${SCHULSONG_TABLE_ID}`);
  console.log('='.repeat(60));
  console.log();
