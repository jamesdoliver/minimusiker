/**
 * Script to create the tracklist_finalized_at field on the Events Airtable table
 * and update the field ID in the codebase.
 *
 * Run with: npx tsx scripts/create-tracklist-finalized-field.ts
 *
 * Creates 1 field:
 *   1. tracklist_finalized_at (DateTime) - when teacher finalized the album tracklist order
 *
...
const FIELDS_TO_CREATE = [
  {
    name: 'tracklist_finalized_at',
    type: 'dateTime',
    description: 'When the teacher permanently locked the album tracklist order. Once set, the tracklist cannot be edited.',
    options: {
      timeZone: 'Europe/Berlin',
      dateFormat: { name: 'iso' },
      timeFormat: { name: '24hour' },
    },
    placeholder: 'fldTODO_REPLACE_ME',
  },
];
...
async function main() {
  ...
  console.log('Creating tracklist_finalized_at field on Events table');
